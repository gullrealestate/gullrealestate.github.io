...
export default function CallErrorModal({ isOpen, onClose }: CallErrorModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex items-center justify-center px-4 sm:px-6">
            {/* Backdrop */}
            <div
                className="absolute inset-0 bg-gruvbox-bg0/80 backdrop-blur-sm transition-opacity"
                onClick={onClose}
            />
            ...
            >
                {/* Header Decoration */}
                <div className="absolute top-0 left-0 right-0 h-24 bg-gradient-to-b from-gruvbox-red/10 to-transparent pointer-events-none" />
                ...
                <div className="p-8 sm:p-10 relative z-0">
                    <div className="flex flex-col items-center text-center">
                        <div className="bg-gruvbox-red/20 p-4 rounded-3xl mb-6 ring-8 ring-gruvbox-red/5">
                            <PhoneOff className="h-10 w-10 text-gruvbox-red" />
                        </div>

                        <h2 className="text-2xl sm:text-3xl font-bold text-gruvbox-fg mb-4">
                            {t.callErrorTitle}
                        </h2>
                        ...
                        <div className="w-full space-y-4">
                            <div className="bg-gruvbox-bg2 p-5 rounded-2xl flex items-start gap-4 text-start border border-gruvbox-bg3">
                                <MessageSquare className="h-6 w-6 text-gruvbox-blue flex-shrink-0 mt-1" />
